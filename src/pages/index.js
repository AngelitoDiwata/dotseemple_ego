...
export default function Home() {
    ...
    const submitDetails = () => {
        ...
        } else if (validator.validate(emailAddress) === false) {
    ...
    return ( <
        div className = 'App w-full h-screen bg-black' > {!loginState && < div className = 'absolute w-full h-screen m-auto bg-black z-50 flex flex-row items-center justify-center space-x-5' >
            <
            input placeholder = "Who are you?"
            className = "text-white w-1/2 md:w-80 transition-all border border-white bg-black rounded-lg hover:outline-white p-2"
            value = { handle }
            onChange = {
                (e) => changeHandler(e, setHandle) }
            /> <
            button onClick = { onLogin }
            className = 'text-5xl text-white mb-2 hover:scale-110 transition-all' > ⦿
            <
            /button> <
            /div>
        } {
            ...
            className = "text-white w-full transition-all border border-white bg-black rounded-lg hover:outline-white p-2"
            value = { walletAddress }
            onChange = {
                (e) => changeHandler(e, setWalletAddress) }
            /> <
            input placeholder = "Email Address"
            className = "text-white w-full transition-all border border-white bg-black rounded-lg hover:outline-white p-2"
            value = { emailAddress }
            onChange = {
                (e) => changeHandler(e, setEmailAddress) }
            /> <
            button onClick = { submitDetails }
            className = 'text-xl border px-3 py-2 rounded-lg mb-2 hover:scale-110 self-end transition-all' >
                Continue <
                /button> <
                /div> <
                /div>
            ...
            loginState && < div className = "absolute z-30 bg-black top-0 w-full m-auto flex flex-col md:flex-row items-start md:items-end justify-between py-5 space-y-3 md:space-x-3 space-x-0 md:space-y-0 px-10" >
                <
                input placeholder = "Search for a handle"
            className = "w-full md:w-1/6 self-start border border-white bg-black rounded-lg outline-white px-3 py-1"
            value = { searchVal }
            onChange = {
                (e) => changeHandler(e, setSearchVal) }
            /> <
            div className = 'w-full md:w-1/4 flex flex-row items-center justify-center space-x-3' >
                <
                input placeholder = "CODE"
            className = "w-full md:w-2/3 border border-white bg-black rounded-lg outline-white px-3 py-1"
            value = { code }
            onChange = {
                (e) => changeHandler(e, setCode) }
            /> <
            button className = "w-full md:w-20 hover:scale-110 transition-all font-semibold border hover:font-neutral-900 hover:border-2 border-white bg-black rounded-lg outline-white px-3 py-1 text-white"
            onClick = { validateEntry } > submit < /button>
            ...
            loginState && < div className = 'bg-black w-full px-2 md:px-20 mx-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 md:gap-5 h-fit pb-32 pt-28 md:pt-16' > {
                    proritizedUserList().filter((item) => item.handle.toLowerCase().includes(searchVal.toLowerCase())).map((data, ind) => {
                            ...
                            return <a key = { ind }
                            href = { `https://twitter.com/${data.handle.replaceAll('@', '')}` }
                            className = { `hover:scale-110 transition-all h-full w-full px-5 py-3 m-auto flex flex-col space-x-5 items-center justify-center text-white my-5 bg-black rounded-full` } >
                                <
